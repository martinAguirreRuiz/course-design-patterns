    ...
    public class GeneratorFileController : Controller
    {
        ...
        public IActionResult CreateFile(int optionFile)
        {
            try
            {
                ...
                if (optionFile == 1) generatorDirector.CreateSimpleJson(content, path);
                else generatorDirector.CreateSimplePipe(content, path);

                var generator = _generatorConcreteBuilder.GetGenerator();
                generator.Save();

                return Json("Archivo generado con éxito");
            }
            catch (Exception ex)
            {
                return BadRequest();
